...
namespace NexusForever.StsServer.Network.Message.Handler
{
    public static class AuthenticationHandler
    {
        ...
        {
            session.EnqueueEvent(new TaskGenericEvent<AccountModel>(DatabaseManager.AuthDatabase.GetAccountAsync(loginStart.LoginName),
                account =>
            {
                if (account == null)
                {
        ...
        public static void HandleRequestGameToken(StsSession session, RequestGameTokenMessage requestGameToken)
        {
            ...
            session.EnqueueEvent(new TaskEvent(DatabaseManager.AuthDatabase.UpdateAccountGameToken(session.Account, guid.ToByteArray().ToHexString()),
                () =>
            {
